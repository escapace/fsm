...
This document defines the semantic model of the default context publication runtime implemented in `src/context-runtime.ts`, with emphasis on `snapshot` and `reconcile`.

The target is the default runtime used by the interpreter when no custom `snapshotContext` or `reconcileContext` option is supplied. The specification is intended to be precise enough to encode in Lean 4 and to refine the coarse context-treatment clause in `lean/README.md` §4.15.

## Status and relation to `lean/README.md`

`lean/README.md` treats context as a value and deliberately abstracts away cloning and identity preservation. That abstraction is appropriate for the existing EFSM proof tranche, but it is too coarse for the context runtime itself.

This document is a narrower specification for the default context runtime only.

- `lean/README.md` remains the semantic specification for machine definition, dispatch, drafts, and composition.
- This document refines `lean/README.md` §4.15 when the runtime uses the default `snapshot` and `reconcile` functions.
...
```text
reconcileSharedObjectValue(current, next, reuse, image):
  if image(next) is defined:
    return image(next)
  if reuse(current) is defined:
    return snapshot(next, image)
...
if SameValue(currentEntry, nextEntry):
  if nextEntry is object-like:
    return reconcileSharedObjectValue(currentEntry, nextEntry, reuse, image)
  else:
    return currentEntry
else:
...
This refines `lean/README.md` §4.6 and §4.15 for the default implementation.
...
State replay and subscription timing remain governed by `lean/README.md`. This document specifies only the context publication operation used at that boundary.
...
The following properties form the recommended theorem set for a later Lean model.
...
If the root values are non-object-like on either side, or if their supported kinds differ, then `reconcile(current, next) = next`.

### R6 — Nested replacement via snapshot

Whenever recursive publication cannot reuse a current subtree because of atom/object mismatch, kind mismatch, or conflicting prior reuse, the result subtree is `snapshot(nextSubtree)` modulo the shared `image` relation.
...
The semantic cost model for later proof work should stay separate from JavaScript engine costs.
...
This specification does not define a minimum-edit objective. Any later optimality claim should be relative to the fixed semantics above, not to tree-edit-distance or graph-edit-distance problems.
...
That is the semantic object the later Lean model should formalize.
