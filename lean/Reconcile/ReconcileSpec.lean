...
/-- The root reconciliation rule. This is the public entry point.
    Returns either the current value unchanged (fast path), the next value
    directly (mismatch), or delegates to kind-specific reconciliation.
...
inductive ReconcileRootSpec {A : Type} [AtomEq A]
  ...
  | curAtomNextRef {a : A} {nr : NextId} :
      ReconcileRootSpec curHeap nextHeap (.atom a) (.ref nr)
        (.ref (.inr ⟨nr.val⟩))  -- next ref lifted to fresh namespace
  ...
  | refNotFound {cr : CurId} {nr : NextId} :
      ...
      ReconcileRootSpec curHeap nextHeap (.ref cr) (.ref nr)
        (.ref (.inr ⟨nr.val⟩))  -- next ref as fresh
...
inductive ReconciledChild {A : Type} [AtomEq A]
  ...
  /-- Fresh snapshot of next (no current counterpart). -/
  | fresh {nv : Value A NextId} {f : FreshId} :
      ReconciledChild curHeap nextHeap w nv (.ref (.inr f))
...
/-- R5 — Root replacement: current ref, next atom ⇒ return next. -/
